...
- **音频技术**：
  ...
  - **MediaRecorder API**：在浏览器端录音并保存为 Blob，供本地回放。
  - **Web Audio API**：绘制可视化的录音波形图（Audio Visualizer）。
- **状态存储**：使用 `localStorage` 保存孩子的关卡进度、录音列表和贴纸册，纯单机即可流畅运行。
...
### 第二阶段：环境初始化与工程搭建 (待开始)
- [ ] 初始化 Git 仓库并连接 GitHub 远程仓库 (`https://github.com/grenvill-create/ReadPictureTellStory.git`)。
- [ ] 初始化 React 现代前端工程项目。
- [ ] 配置视觉风格基础变量（`index.css`）和全局布局。
